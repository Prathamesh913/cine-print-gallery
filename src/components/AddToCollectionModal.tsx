...
import { Check, FolderPlus, Loader2, Lock, Globe, Link2 } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function AddToCollectionModal({ open, onOpenChange, posterId, posterTitle }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="z-[80] border-white/15 bg-[#1c1c1c] text-[#F5F5F5] sm:max-w-md">
        <DialogHeader>
          <DialogTitle style={{ fontFamily: "Poppins, sans-serif" }}>Add to collection</DialogTitle>
          <DialogDescription className="text-white/65">
            {posterTitle ? `Save “${posterTitle}” into a collection.` : "Choose a collection."} Pin
            remains separate — this won’t change your favorites.
          </DialogDescription>
        </DialogHeader>
        ...
        <CreateCollectionModal
          ...
          create={(input) => create({ ...input, posterId })}
        />
      </DialogContent>
    </Dialog>
  );
