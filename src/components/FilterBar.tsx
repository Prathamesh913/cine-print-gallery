    ...
    >
      <div className="mx-auto flex max-w-[1600px] flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:max-w-md">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/55" size={14} />
          <input
            value={query}
        ...
        <div className="flex items-center gap-2 overflow-x-auto whitespace-nowrap scrollbar-hide w-full sm:w-auto sm:ml-auto sm:overflow-x-visible sm:flex-wrap pb-1 sm:pb-0">
          ...
          {activeCount > 0 && (
            <button
              ...
              className="text-xs font-semibold text-white/55 hoverable:hover:text-white active:scale-95 transition-[transform,color] duration-150 ease-[var(--ease-out)] pl-1 shrink-0"
            >
              Clear all
            </button>
          )}
...
function Dropdown({
  ...
  return (
    <div className="relative shrink-0">
      ...
      >
        <span className={value !== "All" ? "text-[#FF6B6B]" : "text-white/65"}>{icon}</span>
        {triggerSwatch}
