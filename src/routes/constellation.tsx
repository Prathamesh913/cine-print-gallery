...
function calculateHarmonies(h: number, s: number, l: number) {
  ...
  return [
    ...
    { name: "Triadic", colors: [format(h), format(h + 120), format(h + 240)] },
    {
      name: "Monochromatic",
      colors: [
        format(h, s, Math.max(15, l - 20)),
        format(h, s, l),
        format(h, s, Math.min(85, l + 20)),
      ],
    },
  ];
}
  ...
  head: () => ({
    meta: [
      { title: "Color Constellation Map — CinePrint" },
      {
        name: "description",
        content:
          "Explore a galaxy of movie poster colors. Posters with similar color palettes gravitate together in an interactive galaxy space map.",
      },
    ],
  }),
...
function ConstellationMap() {
  ...
  const { nodes, links, clusterStats } = useMemo(() => {
    ...
    const processedNodes: PosterNode[] = posters.map((p) => {
      ...
      return {
        ...p,
        displayImage: getPosterImageUrl(p, "gallery"),
    ...
    groupedByCluster.forEach((clusterNodes, clusterId) => {
      ...
      clusterNodes.forEach((nodeA) => {
        ...
        nearest.forEach((n) => {
          ...
          const exists = processedLinks.some(
            (l) =>
  ...
  useEffect(() => {
    ...
    // Build D3 Simulation
    const simulation = d3
      .forceSimulation<PosterNode>(simNodes)
      ...
      .force(
        "link",
        d3
          .forceLink<PosterNode, PosterLink>(simLinks)
          .id((d) => d.id)
          .distance(90)
          .strength(0.04),
      );
    ...
    const render = () => {
      ...
      nodesRef.current.forEach((node) => {
        ...
        const matchesQuery = searchQuery
          ? [node.title, node.artist, node.style].some((s) =>
        ...
        if (k < 0.65) {
          ...
          const cachedImg = imageCache.current[node.displayImage];
          if (cachedImg && cachedImg.complete) {
            ...
            if (!cachedImg) {
              ...
              img.onload = () => {
                imageCache.current[node.displayImage] = img;
              };
              img.onerror = () => {
                if (node.displayImage !== node.image) {
                  img.src = node.image;
                }
              };
              imageCache.current[node.displayImage] = img;
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "#0c0c10", color: "#F5F5F5" }}
    >
      {/* Header Navigation */}
      ...
      >
        ...
        <div className="absolute right-4 bottom-4 flex flex-col gap-2 z-20">
          <div className="flex flex-col gap-1 overflow-hidden rounded-xl border border-white/15 bg-black/60 p-1.5 backdrop-blur-md shadow-2xl">
            <button
              onClick={() => {
                const canvas = canvasRef.current;
            ...
            <button
              ...
              className={`flex h-10 w-10 items-center justify-center rounded-lg transition-colors ${showLinks ? "text-[#FF6B6B]" : "text-white/55 hover:bg-white/10 hover:text-white"}`}
            >
              <Layers size={18} />
            </button>
          </div>
        ...
        <div className="absolute top-4 right-4 z-20 max-w-xs w-full">
          <div className="relative flex items-center rounded-full border border-white/15 bg-black/60 px-3 py-2 backdrop-blur-md shadow-lg">
            <Search className="text-white/55 mr-2 shrink-0" size={16} />
            <input
              type="text"
            ...
            {searchQuery && (
              <button
                ...
                className="text-white/55 hover:text-white ml-1.5"
              >
                <X size={14} />
              </button>
            )}
        ...
        <div
          className={`absolute top-0 bottom-0 left-0 w-80 sm:w-[410px] bg-[#0c0c10]/80 border-r border-white/15 backdrop-blur-xl z-20 shadow-2xl transition-transform duration-500 overflow-y-auto scrollbar-hide ${
          ...
          {selectedPoster ? (
            <div className="p-6">
              ...
              <button
                ...
                className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full bg-white/5 border border-white/15 text-white/60 hover:text-white hover:bg-white/10 transition-all"
              >
                <X size={16} />
              </button>
              ...
              <div className="flex gap-4 mb-6">
                <div
                  className="relative shrink-0 overflow-hidden rounded-md border border-white/15 shadow-lg aspect-[2/3] w-24 sm:w-28"
                  style={{ backgroundColor: "#000000" }}
                >
                  <PosterImage
                    poster={selectedPoster as unknown as Poster}
                    purpose="gallery"
                ...
                <div className="flex flex-col justify-center min-w-0">
                  ...
                  <span
                    className="inline-block self-start text-[10px] uppercase font-mono tracking-widest px-2 py-0.5 rounded border border-white/15 bg-white/5 text-white/70 mt-2.5"
              ...
              <div className="flex gap-2.5 mb-6">
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-white/15 bg-white/5 py-2 px-3 text-xs font-semibold text-white transition hover:bg-white/10"
                >
                  <Compass size={14} />
                  WARP CENTER
                </button>
              ...
              <div className="mb-6 p-4 rounded-xl border border-white/15 bg-white/5">
                <h4 className="text-xs font-mono uppercase tracking-widest text-white/65 mb-3 flex items-center gap-1.5">
                  <Palette size={14} className="text-[#FF6B6B]" /> Dominant Palette
                </h4>
                <div className="grid grid-cols-5 gap-2 h-12 mb-2 rounded-lg overflow-hidden border border-white/15">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {selectedPoster.palette.map((hex, i) => (
                    <button
                      key={i}
                      onClick={() => handleCopyColor(hex)}
                      className="text-[10px] font-mono text-white/65 hover:text-white flex items-center gap-1 bg-white/5 rounded border border-white/5 px-1.5 py-0.5"
                    >
              ...
              <div className="space-y-4 mb-6">
                <h4 className="text-xs font-mono uppercase tracking-widest text-white/65 flex items-center gap-1.5">
                  <Sparkles size={14} className="text-[#FF6B6B]" /> Color Harmonies
                </h4>
                {selectedHarmonies.map((harmony, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between p-2.5 rounded-lg border border-white/5 bg-white/0 hover:bg-white/5 transition-colors"
                  >
                    <span className="text-xs text-white/70">{harmony.name}</span>
                    <div className="flex gap-1 h-6 w-24 rounded border border-white/15 overflow-hidden">
              ...
              <div className="p-4 rounded-xl border border-white/5 bg-white/0">
                <h4 className="text-xs font-mono uppercase tracking-widest text-white/65 mb-2 flex items-center gap-1.5">
                  <Info size={14} /> System coordinates
                </h4>
                <div className="text-[11px] font-mono text-white/55 space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-white/80">
                      {selectedPoster.hsl.s}% / {selectedPoster.hsl.l}%
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div
          className={`absolute top-0 bottom-0 left-0 w-80 sm:w-[410px] bg-[#0c0c10]/60 border-r border-white/15 backdrop-blur-xl z-10 shadow-2xl transition-transform duration-500 overflow-y-auto scrollbar-hide ${
            !selectedPoster ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          <div className="p-6 flex flex-col min-h-full">
            <div className="flex-grow">
              ...
              <p className="text-xs text-white/65 leading-relaxed mb-6 font-mono">
                Explore an alternate visual coordinate plane where film poster designs are
                mathematically grouped by color hues. Clusters form gravity points in a deep galaxy
                system.
              </p>
              ...
              <div className="mb-6 p-4 rounded-xl border border-white/15 bg-white/5">
                ...
                <ul className="text-xs text-white/60 space-y-2 list-none pl-0">
                  <li className="flex gap-2">
                    <span className="text-[#FF6B6B] font-mono">PAN:</span> Drag standard
                    mouse/trackpad to move coordinates.
                  </li>
                  <li className="flex gap-2">
                    <span className="text-[#FF6B6B] font-mono">ZOOM:</span> Scroll wheel or
                    pinch/zoom to dive deeper.
                  </li>
                  <li className="flex gap-2">
                    <span className="text-[#FF6B6B] font-mono">SELECT:</span> Click a glowing star
                    to scan detail readings.
                  </li>
                  <li className="flex gap-2">
                    <span className="text-[#FF6B6B] font-mono">CROSS-FADE:</span> Zoom in closely to
                    transform stars into posters.
                  </li>
                </ul>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-lg font-bold text-white mt-1 pl-4 font-mono">
                          {count}{" "}
                          <span className="text-[10px] text-white/55 font-normal">stars</span>
                        </p>
                      </div>
                    );
                  })}
            ...
            <div className="mt-auto border-t border-white/5 pt-4">
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-white/5 border border-white/15 hover:bg-white/10 text-white font-semibold py-3 text-xs transition"
              >
