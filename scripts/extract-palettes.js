...
function extractPalette(pixels, k = 5, maxIterations = 10) {
  ...
  for (let i = 0; i < k; i++) {
    const idx = Math.min(
      i * step + Math.floor(Math.random() * Math.min(step, 10)),
      pixels.length - 1,
    );
  ...
  try {
    ...
    for (const poster of postersToProcess) {
      ...
      if (
        ...
        paletteCache[poster.id].palette &&
        paletteCache[poster.id].primary
      ) {
        continue;
      }
      ...
      try {
        ...
        console.error(
          `Failed to process image ${poster.imageUrl} for poster ${poster.id}:`,
          err.message,
        );
      }
    }
    ...
    console.log(
      `Cache updated. Added/Updated ${updatedCount} poster palettes. Total items in cache: ${Object.keys(paletteCache).length}.`,
    );
  } catch (err) {
